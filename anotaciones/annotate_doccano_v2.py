...
def build_flexible_pattern(term):
    ...
    pattern = rf"\b({base}|{lemma}(?:s|es|ing|ed|ion|ation|ment|ability|ization)?)\b"
    return re.compile(pattern, flags=re.IGNORECASE)

PRECOMPILED = {
    category: [(term, build_flexible_pattern(term)) for term in terms]
...
def annotate_text(text):
    ...
    used_spans = set()  # evita duplicados

    # Lista de palabras únicas para fuzzy matching
    words = re.findall(r"\b\w+\b", text)
    words_unique = list(dict.fromkeys(words))  # elimina repetidas manteniendo orden

    for category, term_list in PRECOMPILED.items():
        for original_term, pattern in term_list:

            # 1) Match exacto con regex flexible 
            matched_here = False
            for match in pattern.finditer(text):
                span = match.group(0)
                span_range = (match.start(), match.end())
                if span_range in used_spans:
                    continue
                if lightweight_lemma(span) == lightweight_lemma(original_term):
                    annotations.append([match.start(), match.end(), category])
                    used_spans.add(span_range)
                    matched_here = True

            # 2) Fuzzy matching optimizado solo si no hubo match exacto
            if not matched_here:
                target = normalize_text_for_matching(original_term)
                lemma_target = lightweight_lemma(target)

                # solo comparamos palabras que no sean ya coincidencias exactas
                for w in words_unique:
                    ...
                    if sim >= 88:
                        ...
                        if idx != -1:
                            ...
                            if span_range not in used_spans:
                                annotations.append([idx, idx+len(w), category])
                                used_spans.add(span_range)
                        break  # solo un match fuzzy por término
